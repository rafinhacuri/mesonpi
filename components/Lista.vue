...
const tipos = computed(() => {
	const tipos: { nome: string; titulo: string; items: Pagina[] | null }[] = [
		{ nome: 'web', titulo: 'Projetos Equipe WEB', items: null },
		{ nome: 'legados', titulo: 'Projetos Legados', items: null },
		{ nome: 'outros', titulo: 'Outros Projetos', items: null },
	]
	tipos[0].items = Web.value
	tipos[1].items = Legados.value
	tipos[2].items = Outros.value
	return tipos
})
...
<template>
	...
	<div class="dark:bg-slate-900 bg-slate-100">
		<div v-for="tipo in tipos" :key="tipo.titulo">
			<p
				id="equipeWeb"
				class="pt-2 mb-8 text-3xl font-semibold text-center text-black dark:text-white"
			>
				{{ tipo.titulo }}
			</p>
			<div class="overflow-x-auto">
				<table class="w-full table-auto">
					...
					<tbody v-if="tipo.items">
						<tr
							v-for="item in tipo.items" :key="item._id"
