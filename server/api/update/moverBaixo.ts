...
export default defineEventHandler(async (event) => {
	try {
		...
		let objetoPosterior: Document & Pagina | null = null
		if (user !== null) {
			if (tipo === 'web') {
				objeto = await Web.findById(id)
				if (objeto)
					objetoPosterior = await Web.findOne({ ordem: objeto.ordem + 1 })
			}

			else if (tipo === 'outros') {
				objeto = await Outro.findById(id)
				if (objeto)
					objetoPosterior = await Outro.findOne({ ordem: objeto.ordem + 1 })
			}

			else if (tipo === 'legados') {
				objeto = await Legado.findById(id)
				if (objeto)
					objetoPosterior = await Legado.findOne({ ordem: objeto.ordem + 1 })
			}

			if (objeto && objetoPosterior) {
				objeto.ordem++
				await objeto.save()
				objetoPosterior.ordem--
				await objetoPosterior.save()
